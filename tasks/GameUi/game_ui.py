...
class GameUi(BaseTask, GameUiAssets):
    ...
    def ui_goto_page(self, dest_page: Page, confirm_wait=0, skip_first_screenshot=True, timeout: int = 90) -> bool:
        """前往指定page, 自动调用获取当前页面方法, 其他参数同ui_goto
        """
        self.ui_get_current_page()
        return self.ui_goto(dest_page, confirm_wait, skip_first_screenshot, timeout)

    def ui_goto(self, destination: Page, confirm_wait=0, skip_first_screenshot=True, timeout: int = 90) -> bool:
        """
        Args:
            destination (Page):
            confirm_wait:
    ...
    def try_close_unknown_page(self, skip_screenshot: bool = True):
        ...
        for close in self.ui_close:
            if self.appear_then_click(close, interval=3):
    ...
    def _execute_path(self, path: list, timeout_timer):
        ...
        for i, current_page in enumerate(path):
            ...
            if self.ui_current != current_page:
                continue
            self.run_additional(current_page, interval=3, skip_first_screenshot=False)
            # 如果已经是最后一页，不再跳转
            if i == len(path) - 1:
                if len(path) == 1:
            ...
            while not max_wait_timer.reached():
                ...
                if isinstance(button, list):
                    exec_operates = [self.appear_then_operate(btn, interval=3, skip_first_screenshot=False)
                                     for btn in button]
                    if exec_operates[0]:  # 只要第一个成功就跳出
                        break
                if self.appear_then_operate(button, interval=3, skip_first_screenshot=False):
                    break
            else:
            ...
            while not max_wait_timer.reached():
                if timeout_timer.reached():
                    return False
                if self.ui_wait_until_appear(next_page, timeout=8, skip_first_screenshot=False):
